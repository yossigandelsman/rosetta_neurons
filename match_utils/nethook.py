...
class InstrumentedModel(torch.nn.Module):
    ...
    @property
    def is_vit(self):
        return self.model.__class__.__name__ == 'VisionTransformer'

    def retained_layer(self, layer_name=None, clear=False):
        '''
        ...
        if layer_name is None:
            ...
            layer_name = next(self._retained.keys().__iter__())
        result = self._retained[layer_name] 
        if result is not None and self.is_vit:
            hw = int(np.sqrt(result.shape[1] - 1))
            result = rearrange(result[:,1:], 'b (h w) c -> b c h w', h=hw, w=hw)
        if clear:
            self._retained[layer_name] = None
        return result
...
def subsequence(sequential, first_layer=None, last_layer=None, share_weights=False):
    '''
